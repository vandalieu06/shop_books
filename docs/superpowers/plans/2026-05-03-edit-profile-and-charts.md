# Edit Profile & Keep Recharts Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add inline profile editing for users and keep recharts for admin dashboard charts.

**Architecture:** Add `updateMe` endpoint to `users.js`, then modify `Profile.jsx` to support inline editing with form state, save/cancel, and API integration. No changes to charts.

**Tech Stack:** React, useState, lucide-react icons, shadcn form patterns, recharts (unchanged)

---

### Task 1: Add updateMe API Endpoint

**Files:**
- Modify: `frontend/src/api/users.js`

- [ ] **Step 1: Add updateMe function to usersApi**
...
Run: `cat frontend/src/api/users.js`
Expected: File shows `updateMe` function added at the end of the object.

- [ ] **Step 3: Commit**

```bash
git add frontend/src/api/users.js
git commit -m "feat: add updateMe endpoint to usersApi"
```

---

### Task 2: Add Inline Editing to Profile.jsx

**Files:**
- Modify: `frontend/src/pages/Profile.jsx`

- [ ] **Step 1: Rewrite Profile.jsx with inline editing**
				...
				{isEditing ? (
					<div className="space-y-4">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<EditField label="Street" field="address.street" />
							<EditField label="City" field="address.city" />
							<EditField label="Postal Code" field="address.postal_code" />
							<EditField label="Birth Date" field="birth_date" type="date" />
						</div>
...
git commit -m "feat: add inline editing to user profile page"
```

---

### Task 3: Verify Charts Unchanged (Recharts)

**Files:**
- Verify: `frontend/src/components/ui/OrdersChart.jsx`
- Verify: `frontend/src/components/ui/OrdersByStatusChart.jsx`
- Verify: `frontend/package.json`

- [ ] **Step 1: Confirm recharts is still the only chart library**

Run: `grep -E "chart.js|recharts" frontend/package.json`
Expected: Only `recharts` appears, no `chart.js` entry.

- [ ] **Step 2: Confirm chart components use recharts**

Run: `grep "from" frontend/src/components/ui/OrdersChart.jsx frontend/src/components/ui/OrdersByStatusChart.jsx`
Expected: Both show `from "recharts"`.

- [ ] **Step 3: Commit (no changes needed)**

```bash
echo "Charts already use recharts, no changes needed"
```
...
- [x] **Spec coverage:** Edit profile (Task 1 + 2) ✓ | Charts recharts (Task 3) ✓
- [x] **No placeholders:** All code blocks complete, no TODO/TBD
- [x] **Type consistency:** `formData`, `profileData`, `user` usage consistent across Profile.jsx
- [x] **API matches:** `usersApi.updateMe` defined in Task 1, used in Task 2
