...
const add = async (req, res) => {
	try {
		...
		if (!bookId) {
			return res
				.status(400)
				.json({ status: "error", message: "bookId es requerido" });
		}
...
const remove = async (req, res) => {
	try {
		...
		if (!item) {
			return res
				.status(404)
				.json({ status: "error", message: "Libro no encontrado en wishlist" });
		}
		res
			.status(200)
			.json({ status: "success", message: "Libro eliminado de wishlist" });
	} catch (error) {
		res.status(400).json({ status: "error", message: error.message });
	}
};
